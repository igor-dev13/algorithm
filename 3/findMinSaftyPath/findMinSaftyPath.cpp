...
void readData() {
	...
	for (int i = 0; i < m; ++i) {
		...
		adj[u].push_back(v);
		adj[v].push_back(u); // если нужен ориентированный граф убрать
		weight[u].push_back(w);
		weight[v].push_back(w); // если нужен ориентированный граф убрать
	}
...
//процедура восстановления кратчайшего пути по массиву предков
void printWay(int v) {
	if (v == -1) {
		return;
	}
